    ...
    <div class="inline-flex gap-2">
        <div class="inline-flex rounded-lg shadow overflow-hidden h-[1.7rem] bg-gray-100 mb-2">
            {#each modeSort as modeName, i}
                <button class="h-full rounded-lg px-1 {modeName == chosenSort ? "bg-blue-600 text-white" : "bg-gray-100"}" on:click="{()=>{chosenSort = modeSort[i]}}">
                    {modeName}
                </button>
            {/each}
        </div>
        <div class="inline-flex rounded-lg shadow overflow-hidden h-[1.7rem] bg-gray-100 mb-2">
            {#each modeSource as modeName, i}
                <button class="h-full rounded-lg px-1 {modeName == chosenSource ? "bg-blue-600 text-white" : "bg-gray-100"}" on:click="{()=>{chosenSource = modeSource[i]}}">
                    {modeName}
                </button>
            {/each}
        </div>
